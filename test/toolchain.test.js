...
//
// The runtime resolves Dart's web asset convention (`packages/<name>/...`)
// from package_config.json. These exercise the pure resolution logic; the
// end-to-end path is covered by running forge2d's wasm module on Node.

const { pathToFileURL } = require('node:url');

/** Mirrors the resolver in src/runtime.js. */
function resolveWith(roots, url) {
  const match =
    /^package:([A-Za-z_][A-Za-z0-9_]*)\/(.+)$/.exec(url) ??
    /^\/?packages\/([A-Za-z_][A-Za-z0-9_]*)\/(.+)$/.exec(url);
  if (!match) return null;
  const [, name, rest] = match;
  const libUri = roots.get(name);
  return libUri ? new URL(rest, libUri).href : null;
}

const ROOTS = new Map([
  ['forge2d', new URL('lib/', pathToFileURL('/pkgs/forge2d-0.15.1/'))],
]);

test('resolves the packages/ web convention', () => {
  assert.strictEqual(
    resolveWith(ROOTS, 'packages/forge2d/src/backend/wasm/box2d.wasm'),
    pathToFileURL('/pkgs/forge2d-0.15.1/lib/src/backend/wasm/box2d.wasm').href,
  );
});

test('resolves the root-anchored and package: forms too', () => {
  const expected = pathToFileURL('/pkgs/forge2d-0.15.1/lib/a.wasm').href;
  assert.strictEqual(resolveWith(ROOTS, '/packages/forge2d/a.wasm'), expected);
  assert.strictEqual(resolveWith(ROOTS, 'package:forge2d/a.wasm'), expected);
});

test('leaves ordinary URLs alone so real fetch handles them', () => {
  // Regression guard: the polyfill must not swallow network requests.
  for (const url of [
    'https://example.com/x.wasm',
    'http://localhost:8120/v1/task-runs',
    'data:application/wasm;base64,AA==',
  ]) {
    assert.strictEqual(resolveWith(ROOTS, url), null, url);
  }
});

test('returns null for a package that is not installed', () => {
  assert.strictEqual(resolveWith(ROOTS, 'packages/absent/a.wasm'), null);
});
